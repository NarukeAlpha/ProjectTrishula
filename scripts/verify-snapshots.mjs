...
const swiftPath = readArg("--swift");
const outputPath = readArg("--output");
if (!electronPath || !swiftPath) {
  console.error("Usage: node scripts/verify-snapshots.mjs --electron <png> --swift <png> [--output <json>]");
  process.exit(2);
}

const electron = verifyImage("electron", electronPath);
const swift = verifyImage("swift", swiftPath);
const parityFailures = [];
for (const [name] of regions) {
  const delta = Math.abs(electron.regionResults[name].mean - swift.regionResults[name].mean);
  if (delta > 18) {
    parityFailures.push(`${name} average luminance differs too much between Electron and Swift: ${delta.toFixed(2)}`);
  }
}

const trace = {
  verifier: "frontend-snapshot-content",
  isApproved: electron.isApproved && swift.isApproved && parityFailures.length === 0,
  electron,
  swift,
  parityFailures,
};
...
console.log(JSON.stringify({
  ...
  swift: { width: swift.width, height: swift.height },
}));
