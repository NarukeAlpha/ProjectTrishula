...
The `/market-research/jobs` routes use `SERVICE_SHARED_SECRET`, not the Discord agent secret. They have a dedicated registry and do not call a Discord conversation profile. Each job creates an isolated research session with `exa_search`, `exa_read`, `request_chart`, and `update_thesis`. It uses the existing Codex OAuth runtime and does not import, construct, or receive a trading broker. The agent chooses its research, cites saved sources, and discloses missing values. A dedicated numerical feed is not required. Paid request counts survive retries; optional charts use the existing Discord CHART-IMG delivery path.

The optional job `thesisMemory` snapshot contains prior per-stock notes from Convex. `update_thesis` stages an update with a trusted base revision; the runner submits it in `thesisUpdates`, outside the rendered edition. Convex commits notes only after report validation succeeds. Previews, failed reports, and stale revisions cannot change current memory. Long-term reasoning carries forward, but quotes and daily entry levels must be checked again.
