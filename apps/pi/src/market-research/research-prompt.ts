...
Research each configured primary ticker. Keep the primary board to the configured watchlist and at most ten ranked setups. Use sector symbols for confirmation. Add at most three stronger challengers from the configured discovery universe; never fill space with thin or promotional names. Compare only with active durable theses provided in preferences. Without one, use NO PRIOR THESIS. Research is long-only: a falling stock is not a short recommendation, and a reversal needs support, stabilization, and a reclaim rather than a guessed bottom.
