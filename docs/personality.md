...
The current `native-v2` persistence boundary accepts an opaque artifact only for the first checkpoint built from complete raw evidence or for a checkpoint that extends a compatible active native artifact. It deliberately discards opaque output when a request extends a staged candidate or an active portable-only predecessor. Those histories still complete with the readable portable summary. This prevents activation of a delta-only opaque artifact whose earlier evidence is absent. Native coverage for those lineages requires a new, versioned input-basis and provenance contract plus its live acceptance evidence; it is outside this release.
