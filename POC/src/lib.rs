pub mod adapter_contract {
    ...
    pub fn real_session_evidence_required() -> [&'static str; 6] {
        [
            ...
            "Electron live workstation screenshot",
        ]
    }
...
pub mod ibkr_workbench {
    use crate::{
        adapter_contract::EventEnvelope,
        market_read_model::{Bar, HistoricalBarsResponse, QuoteSnapshot},
        runtime_state::BrokerSessionSnapshot,
    };
    use serde::de::DeserializeOwned;
    use serde_json::{json, Value};

    const SOURCE_NAME: &str = "IBKR Gateway/TWS";

    #[derive(Clone, Debug)]
    pub struct IbkrWorkbenchError {
        pub code: &'static str,
        pub message: String,
    }
    ...
    pub fn build_workbench(
        ...
        broker_session: &BrokerSessionSnapshot,
        events: &[EventEnvelope],
    ) -> Result<Value, IbkrWorkbenchError> {
        let symbol = sanitize_symbol(symbol).ok_or_else(|| IbkrWorkbenchError {
            code: "symbolRequired",
            message: "A non-empty stock symbol is required for IBKR market data.".to_string(),
        })?;
        if !broker_session.is_ready() {
            return Err(IbkrWorkbenchError {
                code: "disconnectedGateway",
                message: "IBKR Gateway or TWS is not connected and callback-ready.".to_string(),
            });
        }
        let quote = latest_payload::<QuoteSnapshot>(events, "quote.snapshot", |quote| {
            contract_matches_symbol(&quote.contract.symbol, &symbol)
                || quote
                    .contract
                    .local_symbol
                    .as_deref()
                    .is_some_and(|local_symbol| contract_matches_symbol(local_symbol, &symbol))
        })
        .ok_or_else(|| IbkrWorkbenchError {
            code: "staleData",
            message: format!(
                "No IBKR quote callback is available for {symbol}. Start a market-data subscription before opening the workbench."
            ),
        })?;
        let bars = latest_payload::<HistoricalBarsResponse>(events, "bars.snapshot", |bars| {
            contract_matches_symbol(&bars.contract.symbol, &symbol)
                && bars.bars.iter().any(|bar| contract_matches_symbol(&bar.symbol, &symbol))
        })
        .ok_or_else(|| IbkrWorkbenchError {
            code: "staleData",
            message: format!(
                "No IBKR historical bars callback is available for {symbol}. Start a bars subscription before opening the workbench."
            ),
        })?;
        let snapshot = snapshot_from_ibkr(symbol, broker_session, quote, bars)?;
        Ok(workbench_payload(snapshot, normalize_timeframe(timeframe)))
    }

    fn latest_payload<T>(
        events: &[EventEnvelope],
        event_name: &str,
        matches_symbol: impl Fn(&T) -> bool,
    ) -> Option<T>
    where
        T: DeserializeOwned,
    {
        events.iter().rev().find_map(|event| {
            ...
            let payload = serde_json::from_value::<T>(event.payload.clone()).ok()?;
            matches_symbol(&payload).then_some(payload)
        })
    }

    fn snapshot_from_ibkr(
        symbol: String,
        broker_session: &BrokerSessionSnapshot,
        quote: QuoteSnapshot,
        bars: HistoricalBarsResponse,
    ) -> Result<WorkbenchSnapshot, IbkrWorkbenchError> {
        let bid = parse_number(&quote.bid).ok_or_else(|| IbkrWorkbenchError {
            code: "staleData",
            message: format!("{SOURCE_NAME} quote for {symbol} has an invalid bid."),
        })?;
        let ask = parse_number(&quote.ask).ok_or_else(|| IbkrWorkbenchError {
            code: "staleData",
            message: format!("{SOURCE_NAME} quote for {symbol} has an invalid ask."),
        })?;
        let last = quote
            .last
            .as_deref()
        ...
        let mut parsed_bars = bars
            .bars
            .iter()
            .filter_map(workbench_bar)
            .collect::<Vec<_>>();
        if parsed_bars.is_empty() {
            return Err(IbkrWorkbenchError {
                code: "staleData",
                message: format!("{SOURCE_NAME} bars callback for {symbol} had no usable bars."),
            });
        }
        parsed_bars.sort_by(|left, right| left.timestamp.cmp(&right.timestamp));
        let max_volume = parsed_bars
            .iter()
            .map(|bar| bar.volume)
            .fold(1.0_f64, f64::max);
        let denominator = (parsed_bars.len().saturating_sub(1)).max(1) as f64;
        for (index, bar) in parsed_bars.iter_mut().enumerate() {
            bar.x = index as f64 / denominator;
            bar.volume = 120.0 + (bar.volume / max_volume) * 600.0;
        }
        let previous_close = parsed_bars.first().map(|bar| bar.close).unwrap_or(last);
        let high = parsed_bars.iter().map(|bar| bar.high).fold(last, f64::max);
        let low = parsed_bars.iter().map(|bar| bar.low).fold(last, f64::min);
        let volume = parsed_bars.iter().map(|bar| bar.volume).sum();
        let contract = quote.contract;
        Ok(WorkbenchSnapshot {
            ...
            company: contract
                ...
                .unwrap_or_else(|| contract.symbol.clone()),
            venue: contract
                .primary_exchange
                .clone()
                .filter(|value| !value.is_empty())
                .unwrap_or_else(|| contract.exchange.clone()),
            currency: contract.currency,
            market_data_type: quote.market_data_type,
            quote_timestamp: quote.quote_timestamp,
            fetched_at: quote.captured_at,
            bars_captured_at: bars.captured_at,
            ...
            endpoint: format!(
                "{}:{} client {} {}",
                broker_session.endpoint.host,
                broker_session.endpoint.port,
                broker_session.endpoint.client_id,
                environment_wire_value(broker_session.endpoint.environment)
            ),
        })
    }

    fn workbench_bar(bar: &Bar) -> Option<WorkbenchBar> {
        Some(WorkbenchBar {
            timestamp: bar.timestamp.clone(),
            x: 0.0,
            open: parse_number(&bar.open)?,
            high: parse_number(&bar.high)?,
            low: parse_number(&bar.low)?,
            close: parse_number(&bar.close)?,
            volume: parse_number(&bar.volume).unwrap_or_default(),
        })
    }

    fn workbench_payload(snapshot: WorkbenchSnapshot, active_timeframe: &str) -> Value {
        ...
        json!({
            ...
            "quote": {
                ...
                "ask": price(snapshot.ask),
                "high": price(snapshot.high),
                "low": price(snapshot.low),
                "volume": compact_volume(snapshot.volume),
                "size": snapshot.market_data_type
            },
            "adapter": {
                "connectionState": "ibkrConnected",
                "providerState": "IBKR Market Data",
                "freshness": format!("Quote captured {}", snapshot.fetched_at),
                "paperState": "Paper Not Connected",
                "rollback": "Rollback Active",
                "adapterHealth": "Backend Healthy",
                "alerts": 0,
                "externalEvidence": "IBKR broker callback replay supplied quote and bars"
            },
            "liveSource": {
                ...
                "regularMarketTime": snapshot.quote_timestamp,
                "fetchedAt": snapshot.fetched_at,
                "barsCapturedAt": snapshot.bars_captured_at,
                "barCount": bars.len(),
                "endpoint": snapshot.endpoint
            },
            "watchlists": watchlist(&snapshot, change, change_percent),
            "captures": [],
            "timeframes": ["1m", "5m", "15m", "1h", "D", "W"],
            "activeTimeframe": active_timeframe,
            "bars": bars,
            "levels": [
                {"id": "ask", "label": "Ask", "price": round2(snapshot.ask), "kind": "ask"},
                {"id": "bid", "label": "Bid", "price": round2(snapshot.bid), "kind": "bid"},
            ...
            "markers": [
                {"label": format!("IBKR {}", price(snapshot.last)), "price": round2(snapshot.last), "x": 0.82, "kind": "buy"},
            ...
            "orderTicket": {
                ...
                "quantity": 4,
                "orderType": "Limit",
                "limitPrice": price(snapshot.ask),
                "timeInForce": "Day",
                "account": "IBKR paper (adapter gated)",
                "quoteAge": "IBKR callback",
                "estimatedFill": format!("4 @ {}", price(snapshot.ask)),
                "route": "SMART",
                "venue": snapshot.venue
            },
            "risk": {
                ...
                "marginImpact": "$ 0.00",
                "warnings": [
                    "Market data is sourced only from IBKR Gateway/TWS callbacks recorded by the Rust backend.",
                    "No third-party market-data fallback is available.",
            ...
            "optionsChain": [],
            "diagnostics": [
                format!("IBKR source: {SOURCE_NAME}"),
                format!("Broker endpoint: {}", snapshot.endpoint),
                format!("Quote captured at: {}", snapshot.fetched_at),
                format!("Bars captured at: {}", snapshot.bars_captured_at),
                format!("IBKR bars rendered: {}", bars.len()),
                "Rust endpoint: /v1/workbench/live"
            ]
        })
    }

    fn watchlist(snapshot: &WorkbenchSnapshot, change: f64, change_percent: f64) -> Vec<Value> {
        vec![json!({
            "symbol": snapshot.symbol,
    ...
    fn normalize_timeframe(input: Option<&str>) -> &'static str {
        match input.unwrap_or("5m").to_ascii_lowercase().as_str() {
            "1m" => "1m",
            "15m" => "15m",
            "1h" | "60m" => "1h",
            "d" | "1d" => "D",
    ...
    fn contract_matches_symbol(value: &str, expected: &str) -> bool {
        sanitize_symbol(value).as_deref() == Some(expected)
    }
    ...
    fn environment_wire_value(
        environment: crate::adapter_contract::BrokerEnvironment,
    ) -> &'static str {
        match environment {
            crate::adapter_contract::BrokerEnvironment::IbkrPaper => "ibkrPaper",
            crate::adapter_contract::BrokerEnvironment::IbkrLive => "ibkrLive",
        }
    }
...
pub mod app_settings {
    use crate::{adapter_contract::now_rfc3339, ibkr_workbench};
    ...
    const DEFAULT_WORKSPACE_MODE: &str = "Research";
    ...
    pub struct AppSettings {
        ...
        pub active_workspace_mode: String,
    ...
    pub struct AppSettingsUpdate {
        #[serde(default)]
        pub selected_symbol: Option<Option<String>>,
        pub active_workspace_mode: Option<String>,
    ...
    impl AppSettingsStore {
        ...
        pub fn update(&self, request: AppSettingsUpdate) -> Result<AppSettings, String> {
            let connection = self.open()?;

            if let Some(selected_symbol) = request.selected_symbol {
                match selected_symbol {
                    Some(symbol) => {
                        let symbol = require_symbol(&symbol)?;
                        set_setting(&connection, "selectedSymbol", &symbol)?;
                        upsert_watchlist_symbol(&connection, &symbol, None)?;
                    }
                    None => delete_setting(&connection, "selectedSymbol")?,
                }
            }

            if let Some(mode) = request.active_workspace_mode {
                set_setting(
                    &connection,
                    "activeWorkspaceMode",
                    &normalize_workspace_mode(&mode),
                )?;
            }
            if let Some(timeframe) = request.active_timeframe {
                set_setting(
                    &connection,
                    "activeTimeframe",
            ...
            if let Some(range) = request.active_range {
                set_setting(&connection, "activeRange", &normalize_range(&range))?;
            }

            self.load_with_connection(&connection)
        }

        pub fn upsert_watchlist(&self, request: WatchlistUpsert) -> Result<AppSettings, String> {
            let connection = self.open()?;
            let symbol = require_symbol(&request.symbol)?;
            upsert_watchlist_symbol(&connection, &symbol, request.name.as_deref())?;
            set_setting(&connection, "selectedSymbol", &symbol)?;
            self.load_with_connection(&connection)
        }

        pub fn remove_watchlist_symbol(&self, symbol: &str) -> Result<AppSettings, String> {
            let connection = self.open()?;
            let symbol = require_symbol(symbol)?;
            connection
                .execute(
            ...
            let selected_symbol = setting(&connection, "selectedSymbol")?;
            if selected_symbol.as_deref() == Some(symbol.as_str()) {
                delete_setting(&connection, "selectedSymbol")?;
            }
        ...
        fn load_with_connection(&self, connection: &Connection) -> Result<AppSettings, String> {
            Ok(AppSettings {
                selected_symbol: setting(connection, "selectedSymbol")?,
                active_workspace_mode: setting(connection, "activeWorkspaceMode")?
                    .unwrap_or_else(|| DEFAULT_WORKSPACE_MODE.to_string()),
                active_timeframe: setting(connection, "activeTimeframe")?
                    .unwrap_or_else(|| DEFAULT_TIMEFRAME.to_string()),
                active_range: setting(connection, "activeRange")?
                    .unwrap_or_else(|| DEFAULT_RANGE.to_string()),
                watchlist: watchlist(connection)?,
    ...
    fn default_database_path() -> PathBuf {
        if let Ok(path) = std::env::var("TRADING_SETTINGS_DB") {
    ...
    fn upsert_watchlist_symbol(
        ...
        connection
            .execute(
                "INSERT INTO watchlist_items (symbol, name, position, created_at, updated_at)
                 VALUES (?1, ?2, ?3, ?4, ?4)
                 ON CONFLICT(symbol) DO UPDATE SET
                    name = COALESCE(excluded.name, watchlist_items.name),
                    updated_at = excluded.updated_at",
                params![symbol, name, next_position, now_rfc3339()],
            )
            .map(|_| ())
            .map_err(|error| error.to_string())
    }

    fn watchlist(connection: &Connection) -> Result<Vec<WatchlistEntry>, String> {
    ...
    fn require_symbol(raw: &str) -> Result<String, String> {
        ibkr_workbench::sanitize_symbol(raw)
            .ok_or_else(|| "A non-empty stock symbol is required.".to_string())
    }

    fn normalize_workspace_mode(value: &str) -> String {
        match value {
            "Paper" => "Paper".to_string(),
            "Live Review" => "Live Review".to_string(),
            _ => DEFAULT_WORKSPACE_MODE.to_string(),
        }
    }

    fn normalize_timeframe(value: &str) -> String {
        match value {
            "1m" | "5m" | "15m" | "1h" | "D" | "W" => value.to_string(),
            _ => DEFAULT_TIMEFRAME.to_string(),
        }
    }

    fn normalize_range(value: &str) -> String {
        match value {
            "1D" | "5D" | "1M" | "3M" | "6M" | "YTD" | "1Y" | "5Y" | "All" => value.to_string(),
...
pub mod http_interface {
    use crate::{
        ...
        app_settings, broker_callback_router, broker_protocol,
    ...
    pub struct AppState {
        ...
        pub app_settings: app_settings::AppSettingsStore,
    }
    ...
    impl Default for AppState {
        fn default() -> Self {
            Self {
                ...
                app_settings: app_settings::AppSettingsStore::default(),
            }
        }
    }
    ...
    impl AppState {
        ...
        pub async fn broker_startup(
            ...
            Ok(Self::with_broker_session(evidence.transcript.session))
        }
    ...
    pub fn router_with_state(state: AppState) -> Router {
        Router::new()
            ...
            .route(
                "/v1/app/settings",
                get(app_settings_view).patch(update_app_settings),
            )
            .route("/v1/app/watchlist", post(add_watchlist_symbol))
            .route(
                "/v1/app/watchlist/{symbol}",
                axum::routing::delete(remove_watchlist_symbol),
            )
    ...
    async fn app_settings_view(State(state): State<AppState>) -> impl IntoResponse {
        match state.app_settings.load() {
            Ok(settings) => Json(settings).into_response(),
            Err(message) => (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(AdapterFailure {
                    code: "settingsUnavailable".to_string(),
                    message,
                    request_id: Some("GET /v1/app/settings".to_string()),
    ...
    async fn update_app_settings(
        ...
        Json(request): Json<app_settings::AppSettingsUpdate>,
    ) -> impl IntoResponse {
        match state.app_settings.update(request) {
            Ok(settings) => Json(settings).into_response(),
            Err(message) => (
                StatusCode::BAD_REQUEST,
                Json(AdapterFailure {
                    code: "settingsUpdateRejected".to_string(),
                    message,
                    request_id: Some("PATCH /v1/app/settings".to_string()),
                    retry_after_seconds: None,
                }),
            )
                .into_response(),
    ...
    async fn add_watchlist_symbol(
        State(state): State<AppState>,
        Json(request): Json<app_settings::WatchlistUpsert>,
    ) -> impl IntoResponse {
        match state.app_settings.upsert_watchlist(request) {
            Ok(settings) => Json(settings).into_response(),
            Err(message) => (
                StatusCode::BAD_REQUEST,
                Json(AdapterFailure {
                    code: "watchlistUpdateRejected".to_string(),
                    message,
                    request_id: Some("POST /v1/app/watchlist".to_string()),
                    retry_after_seconds: None,
                }),
            )
                .into_response(),
    ...
    async fn remove_watchlist_symbol(
        ...
        match state.app_settings.remove_watchlist_symbol(&symbol) {
            Ok(settings) => Json(settings).into_response(),
            Err(message) => (
                StatusCode::BAD_REQUEST,
                Json(AdapterFailure {
                    code: "watchlistUpdateRejected".to_string(),
                    message,
                    request_id: Some("DELETE /v1/app/watchlist/{symbol}".to_string()),
                    retry_after_seconds: None,
                }),
            )
                .into_response(),
    ...
    async fn live_workbench_view(
        State(state): State<AppState>,
        Query(query): Query<LiveWorkbenchQuery>,
    ) -> impl IntoResponse {
        let Some(symbol) = query.symbol.as_deref() else {
            return (
                StatusCode::BAD_REQUEST,
                Json(AdapterFailure {
                    code: "symbolRequired".to_string(),
                    message: "Select or enter a stock symbol before requesting live market data."
                        .to_string(),
        ...
        let events = state.event_hub.replay();
        match ibkr_workbench::build_workbench(
            symbol,
            query.timeframe.as_deref(),
            &state.broker_session,
            &events,
        ) {
            Ok(payload) => Json(payload).into_response(),
            Err(error) => {
                let status_code = match error.code {
                    "symbolRequired" => StatusCode::BAD_REQUEST,
                    "disconnectedGateway" => StatusCode::SERVICE_UNAVAILABLE,
                    "missingEntitlement" => StatusCode::FORBIDDEN,
                    "staleData" => StatusCode::CONFLICT,
                    _ => StatusCode::BAD_GATEWAY,
                };
                let retry_after_seconds =
                    matches!(error.code, "disconnectedGateway" | "staleData").then_some(30);
                ...
                (status_code, Json(failure)).into_response()
            }
        }
    }
    ...
    mod tests {
        ...
        #[test]
        fn accepts_nested_bar_subscription_shape_with_client_timeframe() {
        ...
        VerificationTrace {
            ...
            checks: vec![
                check(
                    ...
                    "capability manifest keeps the client wire version",
                ),
                check(
                    "route-count",
                    route_count == 30,
                ...
                check(
                    "failure-codes",
                    failure_count >= 12,
                    "capability manifest exposes client-decodable failure codes",
                ),
                check(
                    "java-wrapper-manifest-shape",
                    java_wrapper_manifest,
                    "capability manifest decodes as the existing IBKR adapter capabilities model",
                ),
                check(
                    "route-safety-flags",
                    route_safety_flags,
        ...
        VerificationTrace {
            ...
            checks: vec![
                ...
                check(
                    ...
                    "protocol callbacks publish a client-decodable connection.status event",
                ),
            ],
        ...
        VerificationTrace {
            ...
            checks: vec![
                ...
                check(
                    "event-replay",
                    event_replay,
                    "decoded callback records publish client-compatible event names through the shared event hub",
                ),
            ],
            evidence: json!(evidence),
        }
        ...
        VerificationTrace {
            ...
            checks: vec![
                ...
                check(
                    "event-replay",
                    event_replay,
                    "decoded field records publish client-compatible event names through the shared event hub",
                ),
                check(
        ...
        VerificationTrace {
            ...
            checks: vec![
                ...
                check(
                    "read-route-shapes",
                    read_routes_decode,
                    "accounts, summaries, and positions expose client-compatible response shapes",
                ),
                check(
        ...
        VerificationTrace {
            ...
            checks: vec![
                ...
                check(
                    ...
                    summaries_positions,
                    "account summary and position callbacks rebuild client-compatible read models",
                ),
                check(
        ...
        VerificationTrace {
            ...
            checks: vec![
                ...
                check(
                    ...
                    "pacingLimit remains an advertised client-decodable failure code",
                ),
                check(
                    "historical-payloads",
                    ticks_sorted,
