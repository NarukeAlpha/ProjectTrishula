...
fn unique_settings_db(name: &str) -> PathBuf {
    let nanos = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .expect("system time")
        .as_nanos();
    std::env::temp_dir().join(format!(
        "agentic-trading-{name}-{}-{nanos}.sqlite",
        std::process::id()
    ))
}
...
#[tokio::test]
async fn live_workbench_rejects_missing_symbol() {
    let (status, json) = request_json("GET", "/v1/workbench/live").await;
    assert_eq!(status, StatusCode::BAD_REQUEST);
    assert_eq!(json["code"], "symbolRequired");
}

#[tokio::test]
async fn live_workbench_requires_ready_ibkr_session() {
    let (status, json) = request_json("GET", "/v1/workbench/live?symbol=AAPL").await;
    assert_eq!(status, StatusCode::SERVICE_UNAVAILABLE);
    assert_eq!(json["code"], "disconnectedGateway");
    assert!(json["message"]
        .as_str()
        .expect("message")
        .contains("IBKR Gateway or TWS"));
}

#[tokio::test]
async fn live_workbench_uses_ibkr_callback_events_only() {
    let state = AppState::connected_fixture();
    let (status, json) = request_json_with_state(
        state,
        "GET",
        "/v1/workbench/live?symbol=AAPL&timeframe=5m",
        [("X-Request-ID", "ibkr-workbench")],
        "",
    )
    .await;
    assert_eq!(status, StatusCode::OK);
    assert_eq!(json["adapter"]["providerState"], "IBKR Market Data");
    assert_eq!(json["liveSource"]["provider"], "IBKR Gateway/TWS");
    assert_eq!(json["liveSource"]["symbol"], "AAPL");
    assert!(json["liveSource"].get("url").is_none());
    assert!(json["bars"].as_array().expect("bars").len() >= 2);
    let payload = serde_json::to_string(&json).expect("payload");
    let legacy_chart_host = format!("{}{}", "query2", ".finance");
    assert!(!payload.contains(&legacy_chart_host));
}

#[tokio::test]
async fn app_settings_start_empty_and_persist_selected_symbol() {
    let state = AppState {
        app_settings: app_settings::AppSettingsStore::new(unique_settings_db("settings")),
        ..AppState::default()
    };

    let (initial_status, initial_json) = request_json_with_state(
        state.clone(),
        "GET",
        "/v1/app/settings",
        [("X-Request-ID", "settings-initial")],
        "",
    )
    .await;
    assert_eq!(initial_status, StatusCode::OK);
    assert_eq!(initial_json["selectedSymbol"], Value::Null);
    assert_eq!(
        initial_json["watchlist"]
            .as_array()
            .expect("watchlist")
            .len(),
        0
    ...
    let (post_status, post_json) = request_json_with_state(
        state.clone(),
        "POST",
        "/v1/app/watchlist",
    ...
    assert_eq!(post_json["selectedSymbol"], "AAPL");
    assert_eq!(post_json["watchlist"][0]["symbol"], "AAPL");

    let (loaded_status, loaded_json) = request_json_with_state(
        state,
        "GET",
        "/v1/app/settings",
        [("X-Request-ID", "settings-loaded")],
        "",
    )
    .await;
    assert_eq!(loaded_status, StatusCode::OK);
    assert_eq!(loaded_json["selectedSymbol"], "AAPL");
}
