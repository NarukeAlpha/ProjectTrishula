...
| `GET` | `/v1/capabilities` | Contract | Returns `apiVersion: "ibkr-local-adapter.v1"`, route metadata, event names, safety gates, graph/ticket capabilities, failure codes, and external evidence gates. |
...
Subscription commands are JSON objects with `action: "subscribe"` and a `subscription` containing `stream: "quote"` or `stream: "bars"` plus positive `conID`. Quote subscriptions receive same-`conID` `quote.snapshot`, `option.quote`, and `ticks.snapshot`; bar subscriptions receive same-`conID` `bars.snapshot` and `ticks.snapshot`; non-market events are broadcast.
